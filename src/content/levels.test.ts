...
describe('levels — Level 3 derives dynamically from LESSONS_BY_GRADE[3] (D9, U6)', () => {
  ...
  test('exam gate unlocks at 3 stars per unit, same rule as Level 1/2 (21 stars for 7 grade-3 units)', () => {
    expect(level3.examGate.unlockAtStars).toBe(LESSONS_BY_GRADE[3].length * 3);
    expect(level3.unitIds).toHaveLength(7);
    expect(level3.examGate.unlockAtStars).toBe(21);
  });
