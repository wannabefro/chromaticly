// DEV / E2E test seam (302.5). Fast-forwards progress so a target unit is ready
// to play with its predecessors already mastered — the prerequisite for
// on-device Maestro coverage of interactions deep in the chain (302.6). Since G6
// U3 nothing is hard-locked, so this seeds *evidence* (completion + mastery),
// never reachability, which every unit now has by default. Pure and
...
/** Complete and 3★-master every lesson (and atom) in `gradeLessons` — the
 ...
 *  below the target's mastered end-to-end (completed/3★, exam recorded cleared —
 *  the state honest play reaches by clearing that grade's exam), then the target's
 *  own grade is walked from its chain root along the authored `unlocks` ordering,
 *  marking each lesson before `targetId` complete, so `targetId` is the frontier
 *  but NOT itself completed, ready to enter. Storage-agnostic and
 ...
 *  the Level 1 exam gate opens — the seam to reach the practice exam (302.2) in an
 ...
 *  yet", so it must not master grade-2 content or clear anything past the Level 1
 *  gate — that would fabricate a cleared exam this seed never runs. */
