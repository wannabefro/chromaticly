...
- **The mark is the chromatic C** (placeholder approved 2026-08-06 — this line previously read "Do not invent a logo"). A ring broken at the lower right, divided into the seven strand hues in lane order. The letter is the name; the seven segments are the seven strands. Alongside it the brand still sets "Chromaticly" in Figtree 800. Rules: no wordmark inside the mark, no new colour, and the ring keeps all seven hues — it is a radar surface under the 2026-07-28 exception, not a one-accent surface. Vector source `design/brand/chromaticly-icon.svg`, exported to `assets/images/`. **This is a placeholder Sam is redesigning externally — do not refine it further in-repo.**
