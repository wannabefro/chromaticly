...
- **The mark is the chromatic C** (approved 2026-08-06 — this line previously read "Do not invent a logo"). A ring broken at the lower right, divided into the seven strand hues in lane order, holding a cream crotchet. The letter is the name and the note the syllabus starts on; the seven segments are the seven strands. Alongside it the brand still sets "Chromaticly" in Figtree 800. Rules: no wordmark inside the mark, no new colour, and the ring keeps all seven hues — it is a radar surface under the 2026-07-28 exception, not a one-accent surface. Source of truth is `assets/images/icon.png`; the note sits wholly inside the counter, so do not lengthen the stem.
