...
| `grade3-anacrusis.yaml` | **Grade 3 anacrusis, anacrusis_recognition MCQ (chromaticly-1v5.3).** Seeds `?seed=anacrusis-3`. Its point is the **pickup-bar notation**: a partial (upbeat) first bar rendered on the light paper card with a complementary short final bar (first + last bar = one whole bar, the ABRSM rule), answered via the generic mcq "How many beats are in the upbeat (anacrusis)?" — abcjs pickup-bar rendering no Jest test can verify. `anacrusis-3` is the new Grade 3 terminal, so seeding auto-scrolls to it and reaching `unit-row-anacrusis-3` is the Level 3 unlock proof. |
