...
| `set-seed-rotation.yaml` | **A replayed lesson is not the same eight questions.** Set seeds are `itemIndex + SET_SIZE * plays`, and `plays` is read through a `useMemo` over the mutable ProgressStore — the React Compiler staleness class Jest's layout-free renderer cannot catch, where a stale read pins every learner to seed offset 0 forever and the suite stays green. Screenshots item 1 on a fresh store, plays the whole set to completion (only a FINISHED set advances `plays`), re-enters and screenshots item 1 again: the two must show different notation. Before the fix, 88 of the curriculum's 262 atoms could never be asked. |
...
| `grade3-intervals.yaml` | **Grade 3 interval quality, interval_naming number+type (chromaticly-1v5.4).** Seeds `?seed=intervals-3`. Its point is the **quality-aware MCQ**: a two-note chord on the light paper card answered via labels like "minor 3rd" / "perfect 5th" / "perfect octave" — and, because grade 3 samples minor-key tonics, a "minor Nth" can be the CORRECT answer (Decision A; the correctness is jest-proven, the label render is on-glass). No `level-node` scroll: seeding auto-scrolls the map to the last unit, so reaching `unit-row-intervals-3` is the unlock proof. On-device this confirmed the chord render + rule-5 FeedbackSheet naming the number/quality misconception with the rendered correct answer. |
| `grade3-ledger-lines.yaml` | **Grade 3 ledger-line note reading, note_naming (chromaticly-1v5.6).** Seeds `?seed=ledger-lines-3`. Its point is the **ledger-line stimulus**: a note drawn on the light paper card one ledger line beyond Grade 2's range (F3/G3/D6/E6 treble, A1/B1/F4/G4 bass), named via the note_naming MCQ — abcjs ledger-line rendering no Jest test can verify. The widened `GRADE_3_SCOPE.pitchRanges` is the reading range these atoms need; incidental-pitch generators (rhythm/metre, interval, scale) stay clamped to the comfortable grade-2 range so only this lesson uses the extreme ledger lines. `ledger-lines-3` is the last unit, so seeding auto-scrolls to it and reaching `unit-row-ledger-lines-3` is the Level 3 unlock proof. On-device this confirmed the bass-clef ledger note rendered below the stave with its play affordance. |
| `grade3-anacrusis.yaml` | **Grade 3 anacrusis, anacrusis_recognition MCQ (chromaticly-1v5.3).** Seeds `?seed=anacrusis-3`. Its point is the **pickup-bar notation**: a partial (upbeat) first bar rendered on the light paper card with a complementary short final bar (first + last bar = one whole bar, the ABRSM rule), answered via the generic mcq "How many beats are in the upbeat (anacrusis)?" — abcjs pickup-bar rendering no Jest test can verify. |
| `grade3-transposition.yaml` | **Grade 3 octave transposition, transposition_input answer stave (2026-07-20-004 plan).** Seeds `?seed=transposition-3`. Its point is the **two-surface composition**: the given melody rendered on the abcjs paper card (`stimulus-music`) above a bespoke tap-to-place answer stave (`transposition-input`) — a stave-placement exercise, not an MCQ, so it mirrors `intervals-stave-input.yaml`'s touch pattern. Places a note by tapping a pitch row (`transposition-pitch-C4`, in range for both the treble and bass answer clef) and confirms the placement ghost is replaced by a real notehead — real-device touch mechanics no Jest test can verify. `transposition-3` is the new Grade 3 terminal (D10 merge rule: appended after `anacrusis-3`), so seeding auto-scrolls to it and reaching `unit-row-transposition-3` is the Level 3 unlock proof. |
